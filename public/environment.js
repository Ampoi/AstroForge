...
export function earthFixed(position,time){
...
function texture(canvas){const t=new THREE.CanvasTexture(canvas);t.wrapS=THREE.RepeatWrapping;t.colorSpace=THREE.SRGBColorSpace;return t;}
...
function earthTexture(){
  ...
  const t=texture(c);
  const ready=fetch('/assets/land.json').then(r=>{if(!r.ok)throw Error('地形データを読み込めません');return r.json();}).then(data=>{
    ...
    for(const [p,color] of [[0,'#e4edf0'],[.1,'#bbc6b1'],[.2,'#78906c'],[.33,'#b6aa7a'],[.42,'#608166'],[.5,'#406f5a'],[.6,'#9d9569'],[.7,'#7c976f'],[.88,'#dfe7df'],[1,'#eef2ef']])terrain.addColorStop(p,color);
...
export class EarthEnvironment{
  constructor(){
    ...
    this.uniforms={earthMap:{value:map},stars:{value:starTexture()},observer:{value:new THREE.Vector3(0,1.001,0)},sunDirection:{value:new THREE.Vector3(-.8,.3,-.5).normalize()},cameraRotation:{value:new THREE.Matrix3()},viewProjection:{value:new THREE.Matrix4()},aspect:{value:1},tanFov:{value:Math.tan(17*Math.PI/180)},globe:{value:0}};
  ...
  render(renderer,camera,globe,rocketCenter){
