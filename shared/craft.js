import {add,mul,sub,dot,inverse3} from './math.js';
...
export function starterCraft(){
  return {name:'Pathfinder 01',parts:[
    ...
    ...Array.from({length:4},(_,i)=>({id:`fin_${i+1}`,type:'fin',parent:'tank_2',offset:-0.34,angle:i*Math.PI/2,group:'fins_1'})),
    ...Array.from({length:4},(_,i)=>({id:`rcs_${i+1}`,type:'rcs',parent:'tank_1',offset:0.32,angle:i*Math.PI/2,group:'rcs_1'}))
  ]};
}
export function twoStageCraft(){
  ...
  craft.parts.push(...Array.from({length:4},(_,i)=>({id:`upper_fin_${i+1}`,type:'fin',parent:'tank_1',offset:-.4,angle:i*Math.PI/2,group:'upper_fins'})));
  return craft;
}
export function validateCraft(input){
  ...
  const parts=input.parts.map(p=>{
    if(!PARTS[p.type]||typeof p.id!=='string'||!/^[a-zA-Z0-9_]{1,48}$/.test(p.id)||ids.has(p.id))throw Error('不正なパーツIDです');
    ids.add(p.id);
    const out={id:p.id,type:p.type};
    if(PARTS[p.type].radial){
      if(typeof p.parent!=='string'||!Number.isFinite(p.angle)||!Number.isFinite(p.offset)||Math.abs(p.offset)>.5)throw Error('取り付け位置が不正です');
  ...
  return {name:input.name.trim(),...(input.rootId!==undefined?{rootId:input.rootId}:{}),parts};
}
// Root of a radial model rests on the hull instead of a fixed floating radius.
export function surfaceRadius(type,offset=0){
...
export function splitCraft(craft,id){
  ...
  const detached=p=>detachedIds.has(p.id)||detachedIds.has(p.parent);
...
export function layoutCraft(craft){
  const positions=new Map();let height=0;
  ...
  return craft.parts.map(p=>{
    ...
    const radius=parent?surfaceRadius(parent.type,p.offset)+({fin:.03,rcs:.04,solar:0}[p.type]||0):0;
...
export function massProperties(craft,fuelFraction=1,monoFraction=1){
...
export function launchIssues(craft){
  const s=craftStats(craft),issues=[];
  try{validateCraft(craft);}catch(e){issues.push(e.message);}
  ...
  for(const p of craft.parts.filter(p=>p.type==='decoupler'))try{splitCraft(craft,p.id);}catch(e){issues.push(e.message);}
