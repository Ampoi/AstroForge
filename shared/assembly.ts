...
export function emptyAssembly(name='新しい機体'): Assembly{
  return {name,assemblyVersion:1,rootId:null,parts:[]};
}
export function toAssembly(craft: Design): Assembly{
...
export function restoreAssembly(value: unknown): Assembly{
  const raw=record(value);
  if(raw.assemblyVersion!==1)return toAssembly(validateCraft(value));
  const input=raw as unknown as Assembly;
...
export function descendantIds(craft: Assembly,roots: Iterable<string>){
...
export function connectedIds(craft: Assembly){return descendantIds(craft,craft.rootId?[craft.rootId]:[]);}
export function movingIds(craft: Assembly,id?: string | null){
  const p=craft.parts.find(p=>p.id===id);
  return descendantIds(craft,p?.group?craft.parts.filter(c=>c.group===p.group).map(c=>c.id):p?[p.id]:[]);
}
export function assembledCraft(craft: Design): Craft{
  ...
  return {name:craft.name,...(craft.rootId&&craft.rootId!==core[0]?.id?{rootId:craft.rootId}:{}),parts:[...core,...parts.filter(p=>PARTS[p.type].radial)].map(p=>{
    const out: Part={id:p.id,type:p.type};
    if(PARTS[p.type].radial){
      const copy=<K extends keyof Part>(key: K)=>{if(p[key]!==undefined)out[key]=p[key];};
      for(const key of ['parent','offset','angle','group','mirror'] as const)copy(key);
    }
    return out;
  })};
}
export function assemblyStats(craft: Design){
...
export function assemblyLayout(craft: Design): LayoutPart[]{
...
export function resolveAssemblyPlacement(craft: Assembly,type: PartType,hit: SurfaceHit | null,{point=[4,0,0],movingId=null,snap=true}: PlacementOptions={}): AssemblyPlacement{
  const moving=craft.parts.find(p=>p.id===movingId),exclude=movingIds(craft,movingId);
  const free: AssemblyPlacement={kind:!craft.rootId?'root':'free',position:[...point],snapped:false};
...
export function placeAssembly(craft: Assembly,type: PartType,placement: AssemblyPlacement | null,{movingId=null,count=1,mirror=false,idFactory=(type,i)=>`preview_${type}_${i}`}: PlacementOptions={}): Assembly{
  ...
  if(!p){
    ...
    for(let i=0;i<count;i++){
      const part: AssemblyPart={id:idFactory(type,i),type,parent:null,position:[...placement.position]};
...
export function removeAssembly(craft: Assembly,id: string){
