...
export function guidance(f,elapsed,state,timeScale=1){
...
export class DemoController{
  ...
  async start(){
    ...
    try{
      await new Promise((resolve,reject)=>{socket.once('error',reject);socket.bind(this.telemetryPort,'127.0.0.1',()=>{socket.off('error',reject);resolve();});});
  ...
  receive(data){
    ...
    if(p.type==='pylon_session'){
      ...
      const session=Object.fromEntries(IDENTITY.map(k=>[k,p[k]]));
  ...
  leaseCommand(action){this.send('pylon_control_authority_command',{action,priority:1,leaseDurationSeconds:2,suppressSas:true});}
  event(text){this.events.push({elapsed:this.elapsed,text});}
  actuators(thrust,rcs){
