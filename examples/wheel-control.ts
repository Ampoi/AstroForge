...
export function options(args: string[]){
  const result={host:'127.0.0.1',commandPort:49011,telemetryPort:49010,seconds:10,wheels:[] as {name:string;motor:number;steering:number;brake:number}[]};
  for(let i=0;i<args.length;i+=2){
    const [key,value]=[args[i],args[i+1]];if(!value)throw Error(`Missing value: ${key}`);
    if(key==='--wheel'){
      const fields=value.split(','),[name]=fields,[motor,steering,brake]=fields.slice(1).map(Number);
      if(fields.length!==4||!name||fields.slice(1).some(v=>!v.trim())||![motor,steering,brake].every(Number.isFinite)||Math.abs(motor)>1||Math.abs(steering)>1||brake<0||brake>1)throw Error('--wheel name,motor[-1..1],steering[-1..1],brake[0..1]');
      if(result.wheels.some(w=>w.name===name))throw Error(`Duplicate wheel: ${name}`);
      result.wheels.push({name,motor,steering,brake});
  ...
  if(!result.wheels.length)throw Error('Specify at least one --wheel name,motor,steering,brake (repeat for each wheel).');
...
export async function run(input:ReturnType<typeof options>){
  ...
  try{
    ...
    try{if(client.owns())for(const wheel of input.wheels)await client.send({type:'pylon_actuator_command',actuatorType:'wheel',name:wheel.name,enabled:true,motor:0,steering:0,brake:1,timeoutSeconds:.3});}
