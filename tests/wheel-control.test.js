...
test('wheel client requires explicit names and bounded user inputs',()=>{
  ...
  assert.throws(()=>options(['--wheel','custom,2,0,0']),/wheel/);
  assert.throws(()=>options(['--wheel','custom,.4,,0']),/wheel/);
  assert.throws(()=>options(['--wheel','custom,.4,0,0','--seconds','NaN']),/seconds/);
  assert.deepEqual(options(['--wheel','custom,.4,-.2,0']).wheels,[{name:'custom',motor:.4,steering:-.2,brake:0}]);
});
test('standalone wheel client sends only explicit targets over UDP without AstroForge',async t=>{
  ...
  await run({host:'127.0.0.1',commandPort:peer.address().port,telemetryPort:port,seconds:.2,wheels:[{name:'arbitrary_right',motor:.25,steering:-.3,brake:0},{name:'arbitrary_left',motor:.2,steering:0,brake:.1}]});
  ...
  assert.equal(commands[0].motor,.25);assert.equal(commands[0].steering,-.3);
  assert.ok(commands.slice(-2).every(p=>p.brake===1&&p.motor===0));
