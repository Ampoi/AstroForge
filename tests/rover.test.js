...
function drive(sim,{motor=0,steering=0,brake=0,expires=1000}={}){
  for(const p of sim.craft.parts.filter(p=>p.type==='wheel'))sim.wheels[p.id]={enabled:true,motor,steering:p.offset>0?steering:0,brake,expires};
}
...
test('front-wheel steering creates yaw and different left/right spring loads',()=>{
  ...
  assert.ok(Math.abs(s.wheelStates[0].compression-s.wheelStates[1].compression)>.015);
});
...
test('wheel UDP commands are named, validated, sequenced and revoked with their lease',()=>{
  ...
  const command={type:'pylon_actuator_command',actuatorType:'wheel',name:'wheel_front_left',enabled:true,motor:.4,steering:.2,brake:0,timeoutSeconds:.5};
  ...
  assert.equal(send({...command,motor:2}).reason,'invalid_wheel_input');
  ...
  const state=packets.find(p=>p.type==='pylon_actuator_state'&&p.name===command.name);assert.equal(state.enabled,true);assert.ok(state.compression>0);
  s.charge=0;const powerless=protocol.telemetry().find(p=>p.type==='pylon_actuator_state'&&p.name===command.name);
  assert.equal(powerless.enabled,false);assert.equal(powerless.motor,0);assert.equal(powerless.brake,1);
