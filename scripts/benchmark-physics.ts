interface Measurement {parts:number;wallMilliseconds:number;physicsStepP95Milliseconds:number}
// Independent processes keep JIT warmup and backend selection comparable.
import {spawnSync} from 'node:child_process';
import {fileURLToPath} from 'node:url';
const cwd=fileURLToPath(new URL('../',import.meta.url)),runs: Record<string,{results:Measurement[]}[]>={js:[],zig:[]};
for(let round=0;round<5;round++)for(const backend of (round%2?['zig','js']:['js','zig'])){
  const result=spawnSync(process.execPath,['--import','tsx','tests/benchmark.js'],{cwd,env:{...process.env,ASTROFORGE_PHYSICS:backend},encoding:'utf8'});
...
const results=[13,80].map(parts=>{
  const measurements: Record<string,Omit<Measurement,'parts'>>={};
