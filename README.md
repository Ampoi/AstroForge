...
画面右上の「UDP 手動デモ」で「手動デモを開始」を押すと、ダッシュボードから操縦できます。

- 出力スライダー（0〜100%）で点火・推力調整。「推力停止」で0%に戻します。
- 飛行中に「段を切り離す」で下段を分離します。分離後は出力0%になり、スライダーで上段を点火できます。
- 「デモ終了」で推力を切り、制御権を返します。追尾機体の変更・VABへの移動でも終了し、画面との接続が途絶えた場合は約3秒で停止します。

姿勢は自動保持されます。サーバー内の独立したUDPクライアントが、選択機体の実際のUDPポートへ指令を送ります。外部コントローラーと同時に同じテレメトリポートは使えません。追加機体は機体メニューでUDPをONにし、「追尾」で選択してください。

自動飛行を試す場合は、手動デモを終了してから別のターミナルで同じフォルダに移動し、付属のUDPコントローラーを実行します。
